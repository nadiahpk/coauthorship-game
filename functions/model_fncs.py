...
#
# updates:
# - tidying errors marked in nvim
# - removed pavlov_memory strategy (doesn't make sense)
...
def calc_next_coauthor_act(prev_ff, prev_fo, prev_of, prev_oo, strategy_name):
    ...
    valid_strategy_names = ["pavlov_c", "pavlov_d", "all_c", "all_d"]
...
def calc_action_gains(
    ...
    coauthor_gains = [
        [
            (
                pg_indicator_fnc(params, 1) * influences_1[i][j] # type: ignore (skipping Nones, so it's fine)
                - pg_indicator_fnc(params, 0) * influences_0[i][j] # type: ignore
            )
            * gammas[i][j]
            * alignments[i][j]
...
def calc_action_gains_positive(params, ID_2_actions, gammas=None, alignments=None):
    ...
    action_gains_positive = {
        ID: [
            [action_gain > 0 for action_gain in focal_action_gains] # type: ignore (won't get None here)
...
def symbolic_transitions_with_errors(pre_2_det, transition_errors_file_name):
    ...
    if str(transition_errors_file_name)[-3:] == "csv":
        df = pd.read_csv(transition_errors_file_name)
    else:
        assert(str(transition_errors_file_name)[-7:] == "parquet")
        df = pd.read_parquet(transition_errors_file_name)
...
def find_stationary_distribution(P, eps, max_pwr=2):
    ...
    eq0s = list()
    pwr_level = max_pwr # loop always runs
