...
# and think about how to do the game with errors

import numpy as np
import itertools as it
import networkx as nx

import examples_fncs_1 as mf
...
action_gains_positive = {
    ID: [
        [
            action_gain > 0
            for action_gain in focal_action_gains
        ]
        for focal_action_gains in mf.calc_action_gains(params, prev_actions, gammas, alignments)
    ]
            ...
            if focal == other:
                ...
                if not other_published:
                    ...
                    if action_gains_positive[prev_ID][focal][other]:
                        ...
                        # if co-authorship does not have a positive switching gain,
                        # co-authorship action depends on: focal's previous action,
                        # other's previous action, and focal's co-author strategy

                        # format for previous action is (focal_prev_act, other_prev_act)
                        prev_act = (prev_actions[focal][other], prev_actions[other][focal])

                        # co-author strategy string
                        coauthor_strat = coauthor_strats[focal]
                        next_act = mf.calc_next_coauthor_act(prev_act, coauthor_strat)
