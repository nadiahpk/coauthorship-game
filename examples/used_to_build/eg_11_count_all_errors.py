# ultimately, I'll want a list of all valid transitions
# and the number of errors and correct between states,
...
for suc_ID in range(nbr_possible_actions):
    ...
    df = pd.DataFrame(
        df_list, 
        columns = ["from_state", "to_state", "is_valid", "nbr_errors", "nbr_correct"]
    )
    df.to_csv("transitions_errors_nbr_players_2.csv", index=False)
